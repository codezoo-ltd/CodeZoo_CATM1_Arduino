...
BG96::BG96(Stream& serial, Stream& debug, uint8_t pwr_pin, uint8_t status_pin):    
	_serial(serial), _debug(debug), _pwr_pin(pwr_pin), _status_pin(status_pin)    
{    
...
int BG96::getRSSI(int* rssi)
{
	...
	if (0 == sendATcmd(szATcmd, resBuffer, sizeof(resBuffer), "+QCSQ: \"CAT-M1\",", 4000))
	{
		Rssi = atoi(resBuffer);
...
int BG96::getTxPower(int* txPower)
{
	...
	if (0 == sendATcmd(szATcmd, resBuffer, sizeof(resBuffer), "+QCPWR: \"CAT-M\",", 4000))
	{
...
int BG96::socketCreate(int service_type, char* remote_address, long remote_port)
{
	char    szCmd[128];
	char	recvBuf[32];
	...
	sprintf(szCmd, "AT+QIOPEN=1,%d,\"%s\",\"%s\",%ld,0,0",
			_nSocket, service[service_type], remote_address, remote_port);
...
int BG96::socketRecv(char* buffer, int bufferSize, int* recvSize, unsigned long timeout)
{
	char    szCmd[32];
	char*   aLine[BG_LINE];  //read up to 20 lines
...
int BG96::sendServicecmd(char* szCmd, char* szResponse, int nResponseBufSize, int* recvSize, unsigned long ulWaitDelay)
{
	int     nRet = 0;
	...
	nRet = readServiceResponseLine(szResponse, nResponseBufSize, recvSize, ulWaitDelay);
		...
		const char* szResponseFilter, unsigned long ulWaitDelay)
{
	...
	nRet = readSckresponseLine(szResponse, nResponseBufSize, szResponseFilter, ulWaitDelay);
...
int BG96::sendATcmd(char* szCmd, char* aLine[], int nMaxLine, unsigned long ulWaitDelay)
{
	int nRet = 0;
...
int BG96::readServiceResponseLine(char* szLine, int nLineBufSize, int* recvSize, unsigned long ulDelay)
{
	int             nbLine = 0;
	char*           aLine[BG_LINE];
...
int BG96::readATresponseLine(char* aLine[], int nMaxLine, unsigned long ulDelay)
{
	int             nbLine = 0;
	Countdown       oCountDown(ulDelay);
	char*           pszSubstr = NULL;
...
#ifdef __AVR__
	vsnprintf_P(szBuf, sizeof(szBuf), (const char *)szTrace, args); // program for AVR
#else
